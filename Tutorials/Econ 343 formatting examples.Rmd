---
title: "Econ 343 Numeric Formatting examples"
output:
...
options(scipen = 999) # prevents r from using scientific notation

knitr::opts_chunk$set(echo = TRUE) # TRUE = commands in r chunks to be echoed in outfile
```

# Prevent Scientific Notation with *options(scipen = 999)*

```{r}
options(scipen = 999) # putting this option in your first r chunk
                      # prevents r from using scientific notation
```

# Use *format()* for additional options

```{r}
## Create variables to use in the formatting examples below

mydata <- hprice1
lm1 <- lm(price ~ bdrms + sqrft ,data = mydata)
bhat1 <- coef(lm1)
bhat_0 <- round(bhat1["(Intercept)"],2)
bhat_1 <- round(bhat1["sqrft"],2)
bhat_2 <- round(bhat1["bdrms"],2)

yhat_225  <- (bhat_0 + bhat_1*2000  + bhat_2 * 4) * 100

```


# Add comma separators using *format()* with "big.mark"

```{r}

yhat_225a <- format(yhat_225,big.mark=",")

```



# Add exactly 2 decimals using *format()* with "nsmall"
```{r}

yhat_225b <- format(round(yhat_225,2), nsmall=2, big.mark=",") 


```

# How they look when knitted

The estimated price is \$`r yhat_225` (yhat_225). 


The estimated price is \$`r yhat_225a` (yhat_225a).


The estimated price is \$`r yhat_225b` (yhat_225b).
