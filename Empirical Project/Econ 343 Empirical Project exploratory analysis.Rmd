...
mygss <- mygss %>%
  mutate(party = case_when(PARTYID == 5 | PARTYID == 6 ~ "Republican",      
         ...
         teamwork = case_when(PARTTEAM == 1 ~ "Team Job",
                              PARTTEAM == 2 ~ "Solo Job")
  )
```
...
``` {r}
mygss <- mygss %>%
  mutate(married  = if_else(MARITAL == 1, "Married", "Not Married"),
         haschild = if_else(CHILDS > 0, "Has Child", "No Children")
  )


```

## Create tables to check accuracy of the recodes


UPDATE: 7.11.2020 - use `drop_rc()` in front of all `cro()` and related table commands to prevent the display of empty rows or columns

```{r}
drop_rc(cro(mygss$PARTYID,mygss$party))
drop_rc(cro(mygss$REGION,mygss$region))
drop_rc(cro(mygss$WRKSTAT,mygss$lfs))
```
...
the new variables created in the last r code chunk.
<<<<<<< HEAD
...
UPDATE: 7.11.2020 - do not use apply_labels() for value labels. See note above - 
use the value labels instead of a number in the `mutate(case_when())` recode command.
>>>>>>> 2e3085885d951aa343891263e08e7d9c13161f19
