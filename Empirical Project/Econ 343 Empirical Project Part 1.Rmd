---
title: "Empirical Project Part 1 ** Add Lastname Firstinitial **"
output:
  html_document:
    number_sections: yes
    toc: yes
...
```{r}
mygss <- read_dta("GSS/GSS2018.dta") %>%
#  select(YEAR,AGE,SEX,RACE,INCOME16,TAX) %>%
   select(year,id,sex,race,income16,tax) %>%
  head(50)

stargazer(as.data.frame(mygss),type="text", 
          digits = 0, 
...
varnames <- names(mygss)

for (x in 1:length(varnames)) {
  varname  = varnames[x]
  varlabel = var_lab(mygss[[x]])
  vallabel = val_lab(mygss[[x]])  %d% NA   # Get rid of labels that have NA values (user missings)

print(paste(varname,varlabel))
  if(!is.null(val_lab(mygss[[x]]))) {
    print(val_lab(mygss[[x]]))
  }
}
```
...
attributes(mygss$tax)
mygss$tax = add_labelled_class(mygss$tax, remove_classes = c("haven_labelled", "spss_labelled", "vctrs_vctr", "double")) 
attributes(mygss$tax)
```
...
mygss <- mygss %>%
  mutate(income16 = case_when(income16 == 98 | income16 == 99) ~ NA)

mygss$tax  <- mis_val(mygss$tax, c(4))

# Dummy variables and collapsed categories
mygss <- mygss %>%
  mutate(tax.dummy = case_when(tax == 1 ~ 1,
                               tax == 2 | tax == 3 ~ 0))
