...
## 
## >>>> STUDENT WORK: Add additional variables to the list below as needed
##
myvars    <- c("SIZE","EDUC","DEGREE","WRKSTAT","HRS1","TAX","PARTYID", "REGION",
               "COURTS","INCOME16")

##
## Standard list of variables to have in the dataset
## 
varsstd <- c("YEAR", "ID", "AGE", "RACE", "SEX", "MARITAL", "CHILDS","WTSSALL")
...
catvars  <- c("RACE", "SEX","DEGREE","WRKSTAT","TAX")

for (x in catvars) {
  table <-  frq(mygss[[x]],weights = mygss$WTSSALL,title=paste(x,var_lab(mygss[[x]])))
  print(table)
}
...
drop_rc(cro_cpct(mygss$tax.toohigh))
...
drop_rc(cro_cpct(mygss$INCOME16))
...
drop_rc(cro_cpct(mygss$INCOME16,mygss$party))%>%
  significance_cpct()
