...
- 'mydata <- mydata %>% mutate( newvar = case_when(oldvar == 0 ~ 0,'
  '                                                oldvar >  0 ~ 1))'
...
$\hat\beta_0 + \hat\beta_{rosneg}$ = 
`r b["(Intercept)"]` + `r b["rosneg"]` = 
**`r b["(Intercept)"]  +  b["rosneg"]`**.
...
**Write-up** Negative *ros* does have a statistically significant impact on CEO salaries. The coefficient on *rosneg* implies that if the CEO’s firm had a negative return on its stock over the 1988 to 1990 period, the CEO salary was predicted to be about `r b["rosneg"]*100`% lower, for given levels of *sales* and *roe*.  The p-value=`r pval["rosneg"]`, which is significant at the 5% level against a two-sided alternative.
...
**Write-up** For each additional year of education, men's wages change on average 
by $\hat{\beta_{educ}}$ = **`r b["educ"]`**. 

The difference for women's wage change is  on average 
by $\hat\{beta_{educ}}$ + $\hat{\beta_{female X educ}}$: **`r b["educ"] + b["female:educ"]`**, 
controlling for experience and tenure.


However, the difference is not statistically significant. 
This is likely because these coefficients pertain to gender differences when educ=0.

# Use mean regressions with dummies

By subtracting the mean of education from *educ* and *female X educ*, the coefficients
are more meaningful:

- the *intercept* represents that average wage for male workers with 12 years of education and no experience

- $\hat\beta_{female}$ is the difference in average wage between men and women with 12 years of education and no experience

- $\hat\beta_{educ}$ is the average increase in men's wages for an additional year of school, , holding experience and tenure constant

- $\hat\beta_{femaleXeduc}$ is the difference in the average increase in women's wages for an additional year of school, holding experience and tenure constant
...
**Write-up** For workers with a typical level of education, educ = `r mean(mydata$educ)` years, the average wage is  **$\hat{\beta_0}$ = ** **`r b["(Intercept)"]`** and the gender difference is:

**$\hat\beta_{female}$ =** **`r  b["female"]`**, both of which are now strongly statistically significant.

The gender difference in the slope coefficient on *educ* is the roughly the same and is still not statistically significant.
...
From Ch 7 Computer Exercise C13: Does the price difference 
between eco- and non-eco-friendly apples affect whether consumers by eco-friendly apples or not?

Frequently, you will create the dummy dependent variable by recoding a quantitative variable. 
...
**Write-up** The primary regressor is the price of apples, eco- and non-eco apples. If the price of eco-friendly apples (*ecoprc*) in dollars increases by, say, 10 cents (.10 dollars), then the probability of buying eco-labeled apples *falls* by about .10 $\cdot$ (`r b["ecoprc"]`) = `r .1*b["ecoprc"]` percentage points.

If *regprc* increases by 10 cents, the probability of buying eco-labeled apples *increases* by about .10 $\cdot$ `r b["regprc"]` = `r .1 * b["regprc"]` percentage points.  

Note: We must assume that the predicted probabilities 
