...
```
# Ch 03 R and Rmarkdown concepts

### R Commands (*) indicates new:
- mutate():     to add variables to your dataset*
- gf_point():  for scatter plots
- cor():       for correlations
- log():       for log transformation
- lm():        for running OLS
- stargazer()  to summarize data and present regression results
- coef():      to save the estimated slope coefficients
- fitted():    to save the yhats*
- resid():     to save the uhats*
- plot(resid()): to plot residuals*

### Rmarkdown Syntax Introduced in this lecture:

- use `$` around math symbols, subscripts, and Greek letters in Rmarkdown
- use `$$` to set math apart on separate line
- use `\` in front of math symbols or Greek letters
