---
title: "Econ 343 Midterm 1 Practice Exam Demo - in class"
output:
...
# lm1 <- ???
lm1 <- lm(final ~ atndrte +  priGPA, data = mydata)
stargazer(lm1,type="text",digits=2)


bhat1   <- coef(lm1)
bhat1_0  <- bhat1["(Intercept)"]
bhat1_atnd <- bhat1["atndrte"]
bhat1_pgpa   <- bhat1["priGPA"]
...
```{r}
lm2 <- lm(final ~ atndrte, data=mydata)
stargazer(lm1,lm2,type="text",digits=2)
