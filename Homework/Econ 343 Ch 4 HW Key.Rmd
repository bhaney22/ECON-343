...
*How do campaign expenditures affect voting outcomes?*

## 2. Economic Model


$$Vote% = f(expenditures, competitor expenditures, if incumbant)$$

## 3. Econometric Model

*{Instructions: Replace depvar and x_ vars with the actual variable names.}*

$$voteA = \beta_0 + \beta_1 * log(expendA) + \beta_2 * log(expendB) + u $$

## 4. Data

Found in the description of the file in the Wooldridge package.
...
```{r}
# Download the STATA dataset from Cengage and then upload the dataset to Rstudio
# change the location and version number of the dataset 
# in the load command, as needed

library(haven)
mydata <- as.data.frame(read_dta("~/20SU ECON 343/Homework/Data/vote1_v2.dta"))
...
# Use the as.data.frame() command to ensure the data can be used by stargazer()
mydata <- as.data.frame(read_dta("~/20SU ECON 343/Homework/Data/401ksubs_v1.dta")) 
stargazer(mydata, type = "text")  
...
num.single <- count(mydata %>% filter(fsize == 1))
...
**`r formatC(as.numeric(num.single),big.mark = ",")`**, or
**`r round(num.single / count(mydata) * 100,1)``%`** of the sample.


### Part (ii)

``` {r}
lm2 <- lm(nettfa ~ inc + age, data = mydata %>% filter(fsize == 1))
bhat2 <- coef(lm2)
